...
if [[ "$recover" = "1" && $(check $2 tophat $Ehome) > 0 ]];then
	...
	if [ "$model" = "fast" ]
		then
		tophat --read-mismatches 2 --mate-inner-dist $insert_size --output-dir 05_TopHat --read-realign-edit-dist 0 --num-threads $threads_num --transcriptome-index $bt_trans $bt_genome $read_1 $read_2  | tee -a $Ehome/run.log
	elif [ "$model" = "medium" ]
		then
		tophat --read-mismatches 2 --mate-inner-dist $insert_size --output-dir 05_TopHat --read-realign-edit-dist 0 --num-threads $threads_num --prefilter-multihits --transcriptome-index $bt_trans $bt_genome $read_1 $read_2  | tee -a $Ehome/run.log
	else
		tophat --read-mismatches 2 --mate-inner-dist $insert_size --output-dir 05_TopHat --read-realign-edit-dist 0 --num-threads $threads_num --prefilter-multihits --coverage-search --transcriptome-index $bt_trans $bt_genome $read_1 $read_2  | tee -a $Ehome/run.log
	fi
